...
const HomePage = () => {
	...
	const { data: recommendedUsersObj = [], isLoading: loadingUsers } = useQuery({
		queryKey: ["users"],
  ...
  const recommendedUsers = recommendedUsersObj.users || [];
	...
	useEffect(() => {
		...
		if (outgoingFriendReqsObj.outgoingRequests && outgoingFriendReqsObj.outgoingRequests.length > 0) {
